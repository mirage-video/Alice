...
def generate(args):
    ...
    logging.info(f"I2V generation job args: {args}")
    logging.info(f"Generation model config: {cfg}")

    input_image = _load_image(args.input_image)
    logging.info(f"Loaded reference image from {args.input_image}")
    ...
    logging.info("Creating AliceImageToVideo pipeline.")
    alice_i2v = AliceImageToVideo(
        config=cfg,
    ...
    logging.info(f"Generating video from image ...")
    video = alice_i2v.generate(
        args.prompt,
    ...
    if rank == 0:
        if args.save_file is None:
            ...
            formatted_prompt = args.prompt.replace(" ", "_").replace("/",
                                                                     "_")[:50]
            suffix = '.mp4'
            args.save_file = f"{args.task}_{args.size.replace('*','x') if sys.platform=='win32' else args.size}_{formatted_prompt}_{formatted_time}" + suffix
